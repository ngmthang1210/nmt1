...
# Lấy thời gian thực gần 0h/12h cho mỗi ngày
...
# Trục thời gian: đặt mốc nhưng ẩn ticklabels
ax1.set_xticks(xtick_times)
ax1.set_xticklabels([""] * len(xtick_times))
ax1.tick_params(axis='x', which='major', length=0)

# Vẽ giờ và ngày bên dưới trục X
for d, t in zip(xtick_dates, xtick_times):
    x = t
    ax1.text(x, -0.07, t.strftime('%H:%M'), transform=ax1.get_xaxis_transform(),
             fontsize=8, ha='center', va='top')
    ax1.text(x, -0.13, d.strftime('%d-%b-%y'), transform=ax1.get_xaxis_transform(),
...
# Xuất PDF
output_path = file_path.replace(".xlsx", "_time_dual_bottom.pdf")
